...
const handleSubscriptionCreated = async (stripeSubscription) => {
    ...
    try {
        ...
        console.log('Subscription customer:', stripeSubscription.customer);
        ...
        if (existingSubscription) {
            console.log('ℹ️ Subscription already exists in database, ID:', existingSubscription._id);
            return;
        }
        ...
        if (userActiveSubscription) {
            console.log('ℹ️ User already has active subscription:', userActiveSubscription._id);
            return;
        }
...
const handleSubscriptionUpdated = async (stripeSubscription) => {
    ...
    try {
        ...
        subscription.status = stripeSubscription.status;
        subscription.currentPeriodStart = safeTimestampToDate(stripeSubscription.current_period_start, subscription.currentPeriodStart);
        subscription.currentPeriodEnd = safeTimestampToDate(stripeSubscription.current_period_end, subscription.currentPeriodEnd);
        subscription.trialStart = safeTimestampToDate(stripeSubscription.trial_start);
        subscription.trialEnd = safeTimestampToDate(stripeSubscription.trial_end);
        subscription.canceledAt = safeTimestampToDate(stripeSubscription.canceled_at);
        subscription.cancelAtPeriodEnd = stripeSubscription.cancel_at_period_end;
...
const handleCheckoutSessionCompleted = async (session) => {
    ...
    try {
        ...
        if (!session.subscription) {
            console.log('ℹ️ No subscription ID in session');
            return;
        }

      
        const stripeSubscription = await stripe.subscriptions.retrieve(session.subscription);
        console.log('Retrieved subscription:', stripeSubscription.id);
        console.log('Subscription customer:', stripeSubscription.customer);
        ...
        console.log('Saving subscription to database...');
        await subscription.save();
        console.log('✅ Subscription created from checkout session, ID:', subscription._id);
...
const handleInvoicePaymentSucceeded = async (invoice) => {
    ...
    try {
        ...
        if (!subscription) {
            console.log('ℹ️ Subscription not found, creating from invoice');
            ...
            try {
                ...
                await subscription.save();
                console.log('✅ Subscription created from invoice payment');
            } catch (createError) {
                console.error('❌ Error creating subscription from invoice:', createError.message);
                return;
            }
        } else {
            subscription.status = 'active';
