...
function makeCalculator(birthYear) {
  return new SiderealAstrologyCalculator('true_sidereal', birthYear || new Date().getFullYear());
}
...
function convertToUTC(data) {
  if (!data.timezone || data.timezone === 'UTC' || data.timezone === '+00:00') {
    return data;
  }

  try {
    // Parse offset string like "-04:00" or "+05:30" into total minutes
    const match = data.timezone.match(/^([+-])(\d{2}):(\d{2})$/);
    if (!match) {
      console.warn(`Unrecognised timezone format: ${data.timezone}, using UTC`);
    ...
    // Build local time in total minutes since epoch, then subtract offset to get UTC
    ...
    console.warn(`Error converting timezone: ${data.timezone}, using UTC`, error);
    return { ...data, timezone: 'UTC' };
  }
}
...
exports.calculateNatalChart = async (req, res) => {
  try {
    ...
    console.log('=== NATAL ROUTE HIT ===');
    console.log('birthData after convertToUTC:', JSON.stringify(birthData));
    const calculator = new SiderealAstrologyCalculator('true_sidereal', birthData.year);
    console.log('calculator.ayanamsa:', calculator.ayanamsa);
    console.log('======================');
...
exports.calculateTransitChart = async (req, res) => {
  try {
    ...
    console.log("WHATS BEING PASSED")
    console.log(birthData)
...
exports.calculateSynastryChart = async (req, res) => {
  try {
    ...
    if (!person2) {
      const calculator = makeCalculator(person1Converted.year);
...
exports.calculateCompositeChart = async (req, res) => {
  try {
    ...
    if (!hasPerson2) {
      const calculator = makeCalculator(person1Data.year);
...
exports.calculateProgressions = async (req, res) => {
  try {
    ...
    const calculator = makeCalculator(birthData.year);
...
exports.calculateSimpleChart = async (req, res) => {
  try {
    ...
    const calculator = makeCalculator(year);
